...
*/
func VerifyMemberAcl(c *gin.Context) {
	var (
	...
	_, err = vrm.GetMemberAcl(getInput, c)
	if err != nil {
		// Expected errors
		if e, ok := tkErr.IsError(err); ok {
			switch e.Code() {
			case cCnt.GRPCMemberAclNotFoundErr.Code():
				statusCode = http.StatusNotFound
				err = tkErr.New(cnt.MidMemberAclNotFoundErr, input.ID)
				utility.ResponseWithType(c, statusCode, err)
				return
			}
		}
		// Unexpected errors
		zap.L().With(
			zap.String(cnt.Middleware, "vrm.GetMemberAcl(...)"),
			zap.String(cnt.RequestID, requestID),
