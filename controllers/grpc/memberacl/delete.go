...
func (m *Method) DeleteMemberAcl(ctx context.Context, input *pb.DeleteInput) (output *pb.DeleteOutput, err error) {
	var (
	...
	whereInput := storCom.DeleteMemberAclWhere{}
	...
	deleteInput := &storCom.DeleteMemberAclInput{
		Where: whereInput,
	}

	deleteOutput, err := storages.Use().DeleteMemberAcl(ctx, deleteInput)
	if err != nil {
		...
		zap.L().With(
			zap.String(cnt.GRPC, "storages.Use().DeleteMemberAcl()"),
			zap.String(cnt.RequestID, requestID),
