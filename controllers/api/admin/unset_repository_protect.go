...
type UnsetRepositoryProtectInput struct {
	ID      string `json:"-"`
	Protect bool   `json:"-"`
	_       struct{}
}

type UnsetRepositoryProtectOutput struct {
	Repository
	_ struct{}
}

func UnsetRepositoryProtect(c *gin.Context) {
	var (
		input = &SetRepositoryProtectInput{
			ID:      c.GetString(cnt.CtxRepositoryID),
			Protect: false,
		}
		output     = &SetRepositoryProtectOutput{}
		err        error
