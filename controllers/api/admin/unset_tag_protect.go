...
type UnsetTagProtectInput struct {
	ID      string `json:"-"`
	Protect bool   `json:"-"`
	_       struct{}
}

type UnsetTagProtectOutput struct {
	Tag
	_ struct{}
}

func UnsetTagProtect(c *gin.Context) {
	var (
		input = &SetTagProtectInput{
			ID:      c.GetString(cnt.CtxTagID),
			Protect: false,
		}
		output     = &SetTagProtectOutput{}
		err        error
