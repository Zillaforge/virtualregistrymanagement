...
// DeleteTag ...
func (o *Operation) DeleteTag(ctx context.Context, input *common.DeleteTagInput) (output *common.DeleteTagOutput, err error) {
	var (
	...
	var id []string
	if deleteErr := whereCascade(o.conn.WithContext(ctx).Model(&tables.Tag{}), &input.Where).Pluck("id", &id).Delete(&tables.Tag{}).Error; deleteErr != nil {
		if sqlErr, ok := deleteErr.(*mysql.MySQLError); ok {
			switch sqlErr.Number {
			// Tag already by reference
			case mariadb_com.ER_ROW_IS_REFERENCED_2:
				...
				err = tkErr.New(cnt.StorageTagInUseErr).WithInner(deleteErr)
				return
			}
		}
		zap.L().With(
