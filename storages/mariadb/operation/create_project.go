...
// CreateProject ...
func (o *Operation) CreateProject(ctx context.Context, input *common.CreateProjectInput) (output *common.CreateProjectOutput, err error) {
	var (
	...
	if createErr := o.conn.WithContext(ctx).Clauses(clause.Returning{}).Create(&input.Project).Error; createErr != nil {
		if sqlErr, ok := createErr.(*mysql.MySQLError); ok {
			...
			case mariadb_com.ER_DUP_ENTRY:
				...
				err = tkErr.New(cnt.StorageProjectExistErr).WithInner(createErr)
				return
			}
		}
		zap.L().With(
	...
	output = &common.CreateProjectOutput{
